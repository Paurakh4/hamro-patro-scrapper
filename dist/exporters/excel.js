...
export class ExcelExporter {
    ...
    async export(data, config) {
        try {
            ...
            throw new ScraperError(ScraperErrorType.FILE_SYSTEM_ERROR, `Failed to export Excel to ${config.outputPath}`, error);
        }
    }
    /**
    ...
    async exportYearFiles(data, outputDir) {
        try {
            ...
            throw new ScraperError(ScraperErrorType.FILE_SYSTEM_ERROR, `Failed to export year Excel files to ${outputDir}`, error);
        }
    }
    /**
     * Populate worksheet with year data
     */
    async populateWorksheet(worksheet, _year, yearData, includeAllFields) {
        // Set up headers
     ...
     */
    async populateMonthWorksheet(worksheet, _year, monthData, includeAllFields) {
        // Set up headers
     ...
     */
    async createSummarySheet(worksheet, data, _includeAllFields) {
        worksheet.addRow(['Hamro Patro Calendar Data Summary']);
        worksheet.addRow([]);
        for (const [yearStr, yearData] of Object.entries(data)) {
            const year = parseInt(yearStr);
            const totalDays = yearData.reduce((sum, month) => sum + month.days.length, 0);
            const totalHolidays = yearData.reduce((sum, month) => sum + month.days.filter((day) => day.isHoliday).length, 0);
 ...
 */
export async function exportToExcel(data, outputPath, includeAllFields = true) {
 ...
 */
export async function exportYearExcelFiles(data, outputDir) {
    const exporter = new ExcelExporter();
