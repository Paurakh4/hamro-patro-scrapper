...
import type { CliOptions, ProgressInfo, ScraperConfig } from './types.js';
...
class HamroPatroCLI {
   ...
   */
  private async handleScrapeCommand(options: CliOptions): Promise<void> {
    try {
   ...
   */
  private async handleConvertCommand(options: any): Promise<void> {
    try {
  ...
  private async handleValidateCommand(options: any): Promise<void> {
    try {
      ...
      const totalDays = years.reduce((sum, year) => {
        return sum + data[year].reduce((monthSum: number, month: any) => monthSum + month.days.length, 0);
      }, 0);
   ...
   */
  private parseScraperConfig(options: CliOptions): ScraperConfig {
   ...
   * Setup progress bar
   */
  private setupProgressBar(config: ScraperConfig): void {
   ...
   * Update progress bar
   */
  private updateProgress(progress: ProgressInfo, config: ScraperConfig): void {
    if (this.progressBar) {
   ...
   */
  private async exportData(data: any, options: CliOptions): Promise<void> {
   ...
   */
  private async exportDataByFormat(
    data: any, 
    format: string, 
    outputPath: string, 
