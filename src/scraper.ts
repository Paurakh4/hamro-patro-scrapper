...
export class HamroPatroScraper {
  ...
  async scrape(
    ...
    if (!this.page) {
      throw new ScraperError(
        ScraperErrorType.NETWORK_ERROR,
        'Scraper not initialized. Call initialize() first.'
      );
    }
    ...
    for (let yearIndex = 0; yearIndex < this.config.years.length; yearIndex++) {
      ...
      for (let monthIndex = 0; monthIndex < this.config.months.length; monthIndex++) {
        ...
        try {
          ...
          throw new ScraperError(
            ScraperErrorType.PARSING_ERROR,
            `Failed to scrape year ${year}, month ${month}`,
            error as Error
          );
        }
   ...
   */
  private async scrapeMonth(year: number, month: number): Promise<CalendarMonth> {
    if (!this.page) {
      throw new ScraperError(
        ScraperErrorType.NETWORK_ERROR,
        'Page not available'
    ...
    try {
      ...
      const days = await this.page.evaluate(() => {
        ...
        function convertNepaliToEnglish(nepaliStr: string): string {
          return nepaliStr
            .split('')
        ...
        ).filter(element => !element.classList.contains('disable'));
      ...
      if (!validation.isValid) {
        throw new ScraperError(
          ...
          `Validation failed for ${year}/${month}: ${validation.errors.join(', ')}`
        );
      }
